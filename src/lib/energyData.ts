...
export type DateRangeOption = 'default' | '7d' | '3d' | '1d'
...
function getDateParams(dateRange: DateRangeOption): Record<string, string> {
  if (dateRange === 'default') return {}
  const days = dateRange === '7d' ? 7 : dateRange === '3d' ? 3 : 1
  // API has ~2 week data lag, so anchor end to 14 days ago
...
export async function getEnergyData({ interval, dateRange }: EnergyParams) {
  ...
  for (const region of REGIONS) {
    ...
    try {
      const priceResp = await apiFetch(`${BASE_URL}/market/network/NEM`, {
        ...
        interval, primary_grouping: 'network_region', ...dateParams,
      })
    ...
    regionData[label] = {
      facilities: facilitySeriesMap.map(f => f.name), rows,
      summary: {
  ...
  return { ok: true, interval, dateRange, data: regionData }
}
