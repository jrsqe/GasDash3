...
interface Payload {
  ok: boolean; interval: string; dateRange: string
          ...
          return (
            <button
              ...
              style={{
                ...
                fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', fontWeight: active ? 600 : 400,
...
function RegionPanel({ region, data }: { region: 'NSW' | 'VIC'; data: RegionData }) {
  ...
  const chartRows = rows.length > 500 ? rows.filter((_, i) => i % 2 === 0) : rows
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
        ...
        <span style={{ color: MUTED, fontFamily: 'DM Mono, monospace', fontSize: '0.72rem' }}>
          {summary.facilityCount} facilities · {rows.length} intervals
        </span>
      </div>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1rem' }}>
        ...
        <ResponsiveContainer width="100%" height={340}>
          <LineChart data={chartRows} margin={{ top: 5, right: 24, left: 0, bottom: 5 }}>
            ...
            <XAxis
              dataKey="datetime" tickFormatter={tickFormatter}
...
export default function DashboardClient() {
  ...
  const fetchData = useCallback(async (iv: IntervalOption, dr: DateRangeOption) => {
    setLoading(true)
    setError(null)
    try {
      const res = await fetch(`/api/energy?interval=${iv}&dateRange=${dr}`)
  ...
  useEffect(() => { fetchData(interval, dateRange) }, [])

  const handleInterval = (iv: IntervalOption) => {
    setInterval(iv)
    fetchData(iv, dateRange)
  }
  const handleDateRange = (dr: DateRangeOption) => {
    setDateRange(dr)
    fetchData(interval, dr)
  }
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '2rem' }}>
        {error ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '5rem 0', gap: '1rem' }}>
            ...
            <span style={{ color: MUTED, fontFamily: 'DM Mono, monospace', fontSize: '0.8rem' }}>
              Fetching data…
            </span>
          ...
          <div style={{ opacity: loading ? 0.5 : 1, transition: 'opacity 0.2s' }}>
            <RegionPanel key={`${activeTab}-${interval}-${dateRange}`} region={activeTab} data={activeData} />
          </div>
        ) : (
